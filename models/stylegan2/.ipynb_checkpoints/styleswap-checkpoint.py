...
#         return input * torch.rsqrt(torch.mean(input * input, axis=1, keepdim=True) + 1e-8)
...
#             out = self.blur(out)
            ...
            in_im = in_im.view(1, batch * in_channel, height, width)
            weight = weight.view(
            ...
            out = F.conv_transpose2d(in_im, weight, padding=0, stride=2, groups=batch)
            _, _, height, width = out.shape
...
#             (1, ), default_initializer=nn.initializer.Constant(0.0))
    ...
    def forward(self, image, noise=None):
        if noise is not None:
            ...
            if isconcat: return torch.cat([image, self.weight * noise], 1) # concat
...
class Generator(nn.Module):
    def __init__(
        ...
        self.to_outtermasks = nn.ModuleList()
    ...
    def forward(
        ...
        if noise is None:
            ...
            for i in range(self.n_mlp + 1):
                size = 2 ** (i+2)
                noise.append(torch.randn((batch, self.channels[size], size, size)))
...
class FullGenerator(nn.Module):
    ...
    def forward(self,
        ...
        out_img, mask = self.generator(outs, return_latents, inject_index, truncation, truncation_latent, 
            input_is_latent, noise=noise[1:],mask = mask)
        if self.outter_mask:
            return out_img, mask,out_img * mask + (1 - mask) * target_img
        else:
            return out_img, mask,out_img